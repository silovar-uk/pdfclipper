...
- PDFページ一覧、キャンバス、設定の3領域に分離
- PDFページ一覧と設定はそれぞれ独立してスクロール
- キャンバスは常に画面内へ固定
- ページ一覧はヘッダーの「ページ」から開閉可能
- コピー、候補追加、書き出し操作を設定欄の下部へ固定
- 書き出し候補一覧は高さを制限し、設定全体が伸び続けない構造

### スマートフォン・タブレット

- キャンバスを画面内へ固定
- 下部ナビゲーションから「ページ」「設定」「候補」「書出」を操作
- ページ一覧と設定はボトムシートとして表示
- PDFページ一覧は横スクロール
- 背景をタップするかEscapeキーでパネルを閉じる

## 基本的な使い方

1. 画像またはPDFを読み込みます。
2. PDFの場合は左側または下部の「ページ」から対象ページを選びます。
3. 必要な範囲をドラッグで選択します。
...
`app.js`が基本的な読み込み・編集・書き出しを担当します。`core/geometry.js`はブラウザに依存しない座標計算を担当し、Node.jsの自動テスト対象です。`layout.js`と`layout.css`は固定レイアウト、パネル表示、操作ボタンの同期を担当します。`advanced/`以下は履歴、回転、余白削除、グリッド、PDFサムネイル、複数候補、ZIP出力などの発展機能を担当します。
